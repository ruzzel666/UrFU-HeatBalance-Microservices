import { Card, Statistic, Row, Col, Segmented, Typography, Divider, Table, Collapse, Tag } from 'antd';
import {
  FireOutlined,
  ThunderboltOutlined,
  ExperimentOutlined,
...
/**
 * Компонент отображения результатов расчёта камерной печи.
 * @param {{ data: object, onBack: () => void }} props
 */
export default function ResultsView({ data, onBack }) {
  ...
  /* ===== Fuel combustion data for collapsible tables ===== */
  ...
  const collapseItems = [
    {
      ...
      children: (
        <Table
          columns={combustionColumns}
          dataSource={aerodynamicsData}
          pagination={false}
          size="small"
          bordered
        />
  ...
  return (
    <div className="results-view">
      {/* Back button */}
      <a className="back-link" onClick={onBack} id="btn-back-to-form">
      ...
      <Card
        className="results-section animate-fade-in-up delay-1"
        title={
