...
public class CreateChamberKilnModel : PageModel
{
    private readonly IHeatBalanceBackend _backend;
    private readonly HeatBalanceRunService _runService;

    public CreateChamberKilnModel(IHeatBalanceBackend backend, HeatBalanceRunService runService)
    {
        _backend = backend;
    ...
    [BindProperty]
    public ChamberKilnInput Input { get; set; } = new();

    public void OnGet()
    {
        Input.FuelLowerHeatingValueKJPerKg = 42000;
        Input.HeatLossFraction = 0.2;
    }
    ...
    public async Task<IActionResult> OnPostAsync()
    {
        ...
        await _backend.CreateDataSetAsync(userId, new InputDataSet
        {
            Name = Name,
            Comment = Comment,
            CalculationType = CalculationType.ChamberKiln,
            InputJson = _runService.SerializeInput(CalculationType.ChamberKiln, Input)
        });
