...
public class EditDrumDryerModel : PageModel
{
    private readonly IHeatBalanceBackend _backend;
    private readonly HeatBalanceRunService _runService;

    public EditDrumDryerModel(IHeatBalanceBackend backend, HeatBalanceRunService runService)
    {
        _backend = backend;
    ...
    [BindProperty] public DrumDryerInput Input { get; set; } = new();
    ...
    public async Task<IActionResult> OnGetAsync()
    {
        ...
        if (ds.CalculationType != CalculationType.DrumDryer) return RedirectToPage("./Edit", new { id = Id });
        Name = ds.Name; Comment = ds.Comment; Input = (DrumDryerInput)_runService.DeserializeInput(ds);
        return Page();
    }
    ...
    public async Task<IActionResult> OnPostAsync()
    {
        ...
        ds.InputJson = _runService.SerializeInput(CalculationType.DrumDryer, Input);
