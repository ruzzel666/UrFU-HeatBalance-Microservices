...
public class EditChamberKilnModel : PageModel
{
    private readonly IHeatBalanceBackend _backend;
    private readonly HeatBalanceRunService _runService;

    public EditChamberKilnModel(IHeatBalanceBackend backend, HeatBalanceRunService runService)
    {
        _backend = backend;
    ...
    [BindProperty] public ChamberKilnInput Input { get; set; } = new();
    ...
    public async Task<IActionResult> OnGetAsync()
    {
        ...
        if (ds.CalculationType != CalculationType.ChamberKiln) return RedirectToPage("./Edit", new { id = Id });
        Name = ds.Name; Comment = ds.Comment; Input = (ChamberKilnInput)_runService.DeserializeInput(ds);
        return Page();
    }
    ...
    public async Task<IActionResult> OnPostAsync()
    {
        ...
        ds.InputJson = _runService.SerializeInput(CalculationType.ChamberKiln, Input);
